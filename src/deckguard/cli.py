...
@click.option("--rules", "rules_path", type=click.Path(exists=True, dir_okay=False), default=None)
def redesign(deck: str, out_path: str, model: str, effort: str, notes: Optional[str], template: Optional[str], rules_path: Optional[str]):
    """Rebuild DECK from scratch onto the org template, using Claude to
    judge which slide kind (cover/content/quote/stat/timeline/...) each
    slide's content should become.

    This is the AI-assisted counterpart to `create`: instead of a
    hand-written YAML outline, the outline is generated by asking Claude
    to read each eligible slide's extracted text and map it onto
    compose.py's own outline schema -- then it's built through the exact
    same deterministic pipeline `create` uses (same layout selection,
    same final brand-compliance pass), so nothing about color, font, or
    approved-layout judgment is ever left to the model. A slide with a
    table, chart, embedded object, or more content than any layout can
    hold is never guessed at -- it's skipped and reported, same as
    `retemplate`.
    ...
    try:
        compose_result, redesign_result = redesign_deck(
            deck, str(out), model=model, effort=effort, notes=notes,
